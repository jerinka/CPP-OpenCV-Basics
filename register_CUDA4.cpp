...
public:
		...
		const Mat& src5, Mat& dst)
	{
		...
		den__gpu.download(den_);
		src1Divided__gpu.download(src1Divided_);
		src2Divided__gpu.download(src2Divided_);
		
		dst_gpu.download(dst);
		temp__gpu.download(temp_);
	}
	...
	void project_onto_jacobian_ECC(const Mat& src1, const Mat& src2, Mat& dst)
	{
		...
		*/
		...
		dst1_gpu.download(dst1);
		dst2_gpu.download(dst2);
		//dst_gpu.download(dst);
		//cout<<"test7"<<endl;
	...
	int registerimg(Mat & im1, Mat & im2, Mat & inputMask, Mat & im2_aligned)
	{
		...
		Mat jacobian, hessian, hessianInv, imageProjection, templateProjection, errorProjection, imageProjectionHessian, deltaP, error;
		...
		const int ws = src_gpu.cols;
		const int hs = src_gpu.rows;
		const int wd = dst_gpu.cols;
		const int hd = dst_gpu.rows;
		...
		cv::cuda::GpuMat templateZM_gpu = cv::cuda::GpuMat(hs, ws, CV_32F);// to store the (smoothed)zero-mean version of template
		cv::cuda::GpuMat templateFloat_gpu = cv::cuda::GpuMat(hs, ws, CV_32F);// to store the (smoothed) template
		cv::cuda::GpuMat imageFloat_gpu = cv::cuda::GpuMat(hd, wd, CV_32F);// to store the (smoothed) input image
		cv::cuda::GpuMat imageWarped_gpu = cv::cuda::GpuMat(hs, ws, CV_32F);// to store the warped zero-mean input image
		cv::cuda::GpuMat imageMask_gpu = cv::cuda::GpuMat(hs, ws, CV_8U); //to store the final mask
		...
		cv::cuda::GpuMat hessian_gpu = cv::cuda::GpuMat(numberOfParameters, numberOfParameters, CV_32F);
		cv::cuda::GpuMat hessianInv_gpu = cv::cuda::GpuMat(numberOfParameters, numberOfParameters, CV_32F);
		cv::cuda::GpuMat imageProjection_gpu = cv::cuda::GpuMat(numberOfParameters, 1, CV_32F);
		cv::cuda::GpuMat templateProjection_gpu = cv::cuda::GpuMat(numberOfParameters, 1, CV_32F);
		cv::cuda::GpuMat imageProjectionHessian_gpu = cv::cuda::GpuMat(numberOfParameters, 1, CV_32F);
		cv::cuda::GpuMat errorProjection_gpu = cv::cuda::GpuMat(numberOfParameters, 1, CV_32F);
		...
		inputMask_gpu.download(inputMask);
		src_gpu.download(src);
		dst_gpu.download(dst);
		warpMatrix_gpu.download(warpMatrix);
		map_gpu.download(map);


		preMaskFloat_gpu.download(preMaskFloat);

		//jacobian_gpu.download(jacobian);
		hessian_gpu.download(hessian);
		hessianInv_gpu.download(hessianInv);
		imageProjection_gpu.download(imageProjection);
		templateProjection_gpu.download(templateProjection);
		imageProjectionHessian_gpu.download(imageProjectionHessian);
		errorProjection_gpu.download(errorProjection);
		...
		for (int i = 1; (i <= numberOfIterations) && (fabs(rho - last_rho) >= termination_eps); i++)
		{
			...
			cv::cuda::GpuMat templateZM_gpu = cv::cuda::GpuMat(templateZM_gpu.rows, templateZM_gpu.cols, templateZM_gpu.type(), double(0));
			...
			imageMask_gpu.download(imageMask);
			templateFloat_gpu.download(templateFloat);
			imageFloat_gpu.download(imageFloat);
			gradientX_gpu.download(gradientX);
			gradientY_gpu.download(gradientY);
			preMask_gpu.download(preMask);
			gradientXWarped_gpu.download(gradientXWarped);
			gradientYWarped_gpu.download(gradientYWarped);
			Xgrid_gpu.download(Xgrid);
			Ygrid_gpu.download(Ygrid);
...
int main()
{
	...
	Mat sample_img = imread("/home/nvidia/Documents/registration/cuda_reg/1_new.png");
	...
	RegisterImagesCuda regimgcuda;
	//RegisterImagesCputoCuda regimgcputocuda;

	std::map< String, int>::iterator it;
	

	String name = "regimgcuda";
	TimerElement temp(name);
	timer_dic["regimgcuda"] = temp;
	...
	//timer.get_total_time("regimgcuda");
	timer.get_average_time("upload");
	timer.get_call_count("upload");
	timer.get_average_time("cvtColor");
	timer.get_average_time("regimgcuda");
